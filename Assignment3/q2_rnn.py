...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        sentence = sentence[:max_length-padN] + [zero_vector]*padN
        labels = labels[:max_length-padN] + [zero_label]*padN
        masks = [True]*(max_length-padN) + [False]*padN
        ret.append((sentence, labels, masks))
        ### END YOUR CODE ###
    return ret
...
class RNNModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        self.input_placeholder = tf.placeholder(
            tf.int32, [None, self.max_length, Config.n_features], name="input")
        self.labels_placeholder = tf.placeholder(
            tf.int32, [None, self.max_length], name="label")
        self.mask_placeholder = tf.placeholder(
            tf.bool, [None, self.max_length], name="mask")
        self.dropout_placeholder = tf.placeholder(
            tf.float32, name="drop")
        ### END YOUR CODE
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope('Layer1'):
            ...
            b2 = tf.get_variable(name='b2', shape=[Config.n_classes],
                                initializer=tf.contrib.layers.xavier_initializer())
    ...
    def add_loss_op(self, preds):
        ...
        loss = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=masked_logits, labels=masked_labels)
    ...
    def add_training_op(self, loss):
        ...
        adam_optimizer = tf.train.AdamOptimizer(Config.lr)
